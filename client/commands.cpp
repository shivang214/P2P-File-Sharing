...
int list_groups(int sock){
    ...
    std::vector<string> grps = splitString(string(reply), "$$");
...
int list_requests(int sock){
    ...
    read(sock, reply, 3*SIZE);
    if(std::string(reply) == "**err**") return -1;
    if(std::string(reply) == "**er2**") return 1;


    std::vector<std::string> requests = splitString(string(reply), "$$");

    {size_t i=0;
while(requests.size()-1 > i){
	{
...
void list_files(int sock){
    ...
    bzero(buf, 1024);
    read(sock, buf, 1024);
    vector<string> listOfFiles = splitString(std::string(buf), "$$");
