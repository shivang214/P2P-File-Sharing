...
std::string logFileName, tracker1_ip, tracker2_ip, peer_ip, seederFileName;
uint16_t peer_port, tracker1_port, tracker2_port;
bool loggedIn;
std::unordered_map<string, unordered_map<string, bool>> isUploaded; // group -> filename -> bool
std::unordered_map<string, vector<int>> fileChunkInfo;
vector<vector<string>> curDownFileChunks;
std::unordered_map<string, string> fileToFilePath;
vector<string> curFilePiecewiseHash;
std::unordered_map<string, string> downloadedFiles;
...
int main(int argc, char* argv[]){
    ...
    while(true){ 
        std::cout << ">> ";
        std::string inptline, s;
        std::getline(cin, inptline);

        if(inptline.length() < 1) continue;
        
        stringstream ss(inptline);
       std:: vector<string> inpt;
        while(ss >> s){
            inpt.push_back(s);
        } 
        ...
        if(inpt[0] != "login" && inpt[0] != "create_user" && !loggedIn){
            std:: cout << "Please login / create an account" << endl;
                continue;
        }
